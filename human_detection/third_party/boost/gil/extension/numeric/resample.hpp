...
struct resample_pixels_fn
  ...
  template <typename SrcView, typename DstView> GIL_FORCEINLINE
  void apply_compatible(const SrcView& src, const DstView& dst)  const {
...
void resample_pixels(const any_image_view<Types1>& src, const V2& dst,
                     const MapFn& dst_to_src, Sampler sampler=Sampler()) {
  apply_operation(src,bind(detail::resample_pixels_fn<Sampler,MapFn>
                           (dst_to_src,sampler), _1, dst));
}
...
void resample_pixels(const V1& src, const any_image_view<Types2>& dst,
                     const MapFn& dst_to_src, Sampler sampler=Sampler()) {
  apply_operation(dst,bind(detail::resample_pixels_fn<Sampler,MapFn>
                           (dst_to_src,sampler), src, _1));
}
...
void resample_pixels(const any_image_view<SrcTypes>& src,
                     ...
                     const MapFn& dst_to_src, Sampler sampler=Sampler()) {
  apply_operation(src,dst,detail::resample_pixels_fn<Sampler,MapFn>(dst_to_src,
                                                                    sampler));
}
...
void resample_subimage(const SrcMetaView& src, const DstMetaView& dst,
  ...
  matrix3x2<double> mat =
    ...
    matrix3x2<double>::get_translate(src_min_x + src_width/2.0 + 0.0001,
                                     src_min_y + src_height/2.0 + 0.0001);
...
void resize_view(const SrcMetaView& src, const DstMetaView& dst,
  ...
  resample_subimage(src,dst,0,0,double(src.width()),double(src.height()),0,
                    sampler);
}
