...
enum convolve_boundary_option  {
  ...
  convolve_option_extend_padded,  /// assume the src bounds padded already
  convolve_option_extend_zero,    /// assume the src bounds to be zero
  convolve_option_extend_constant /// assume the src bounds to be bound value
};
...
private:
  std::size_t _size;
public:
  explicit correlator_n(std::size_t size_in) : _size(size_in) {}
...
template <typename PixelAccum,typename SrcView,typename Kernel,typename DstView>
GIL_FORCEINLINE
void correlate_rows_fixed(const SrcView& src, const Kernel& ker,
                          const DstView& dst,
                  convolve_boundary_option option=convolve_option_extend_zero) {
  detail::correlate_rows_imp<PixelAccum>(src,ker,dst,option,
...
template <typename PixelAccum,typename SrcView,typename Kernel,typename DstView>
GIL_FORCEINLINE
void correlate_cols_fixed(const SrcView& src, const Kernel& ker,
    const DstView& dst,
    convolve_boundary_option option=convolve_option_extend_zero) {
  correlate_rows_fixed<PixelAccum>(transposed_view(src),ker,transposed_view(dst)
                                   ,option);
}
...
template <typename PixelAccum,typename SrcView,typename Kernel,typename DstView>
GIL_FORCEINLINE
void convolve_rows_fixed(const SrcView& s, const Kernel& ker, const DstView& d,
                  convolve_boundary_option option=convolve_option_extend_zero) {
  correlate_rows_fixed<PixelAccum>(s,reverse_kernel(ker),d,option);
}
...
template <typename PixelAccum,typename SrcView,typename Kernel,typename DstView>
GIL_FORCEINLINE
void convolve_cols_fixed(const SrcView& s, const Kernel& ker, const DstView& d,
                  convolve_boundary_option option=convolve_option_extend_zero) {
  convolve_rows_fixed<PixelAccum>(
    transposed_view(s),ker,transposed_view(d),option);
}
