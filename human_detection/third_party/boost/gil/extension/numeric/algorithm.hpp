...
/// \brief Returns the reference proxy associated with a type that has a
///   \p "reference" member typedef.
///
...
inline DstIterator assign_pixels(SrcIterator src,SrcIterator src_end,
                                 DstIterator dst) {
  for_each(src,src_end,dst,pixel_assigns_t<typename pixel_proxy<
...
struct inner_product_k_t {
  ...
  static _Tp apply(_InputIterator1 __first1,
    ...
    return inner_product_k_t<Size-1>::template
      apply(__first1+1,__first2+1,__init, __binary_op1, __binary_op2);
  }
};
...
_Tp inner_product_k(_InputIterator1 __first1,
                    ...
                    _BinaryOperation1 __binary_op1,
                    _BinaryOperation2 __binary_op2) {
  return detail::inner_product_k_t<Size>::template
    apply(__first1,__first2,__init,__binary_op1, __binary_op2);
}
...
inline DstIterator correlate_pixels_n(SrcIterator src_begin,SrcIterator src_end,
  ...
  typedef typename pixel_proxy<typename
    std::iterator_traits<SrcIterator>::value_type>::type PIXEL_SRC_REF;
  typedef typename pixel_proxy<typename
    std::iterator_traits<DstIterator>::value_type>::type PIXEL_DST_REF;
